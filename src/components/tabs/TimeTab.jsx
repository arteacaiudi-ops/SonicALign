...
export default function TimeTab() {
  const { getCircularBufferSlice, getSampleRate, playReferenceSignal, isRunning, start, stop, selectedDevice, autoGainNormalize } = useAudioEngine();
  ...
  return (
    <div className="flex flex-col h-full bg-black font-mono overflow-hidden">
      <div className="bg-zinc-950 p-2 border-b border-zinc-900 flex justify-between items-center gap-1 shrink-0 shadow-lg overflow-x-hidden">
        ...
        <div className="flex gap-1 shrink-0">
          <button onClick={() => autoGainNormalize(-15)} className="p-2 rounded border border-zinc-800 text-zinc-400"><Crosshair size={14}/></button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative"><RollingGraph data={isFrozen ? frozenData : getCircularBufferSlice(getSampleRate() * timeWindow)} threshold={threshold} markers={markers} isFrozen={isFrozen}/></div>
      </div>
    </div>
  );
}
