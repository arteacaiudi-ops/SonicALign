...
import { Play, Square, LayoutGrid, TrendingDown, BarChart3, Waves, SlidersHorizontal, Activity, Crosshair, AlertCircle } from 'lucide-react';
...
export default function AmbienceTab() {
  ...
  const handleAutoGain = () => peakHoldAutoGain(5000);
  ...
  const processAcoustics = useCallback((samples) => {
    ...
    if (pulseIdx !== -1) {
      ...
      if (early > 0) setMetrics({ c50: 10 * Math.log10(early / (late || 0.0001)), rt60: 0.3 + (late / early) * 15 });
    }
  }, [getSampleRate, threshold]);
    ...
    <div className="flex flex-col h-full bg-black font-mono overflow-hidden">
      <div className="flex bg-zinc-950 border-b border-zinc-900 overflow-x-auto no-scrollbar px-1 py-1 gap-1 shrink-0">
        ...
        {['summary', 'etc', 'bands', 'waterfall', 'eq'].map(id => (
          <button key={id} onClick={() => setActiveView(id)} className={`px-3 py-3 text-[9px] font-bold uppercase border-b-2 transition-all ${activeView === id ? 'text-neon-blue border-neon-blue' : 'text-zinc-600 border-transparent'}`}>{id}</button>
        ))}
      </div>
      ...
      {isListening && (
        <div className="bg-neon-blue/20 p-3 flex items-center gap-3 animate-pulse border-b border-neon-blue/30">
          ...
          <div className="text-[10px] text-white font-bold leading-tight">
            NÃO MOVA O DISPOSITIVO.<br/>DISPARE O SINAL E AGUARDE A FINALIZAÇÃO.
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-2 gap-2 overflow-y-auto">
          ...
          <div className="p-3 bg-zinc-900/50 border border-zinc-800 rounded-xl flex justify-between items-center shrink-0">
             <button onClick={handleAutoGain} className="p-2 rounded border border-zinc-800 text-zinc-400"><Crosshair size={14}/></button>
             <button 
